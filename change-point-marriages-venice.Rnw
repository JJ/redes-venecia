...
\begin{landscape}
<<change.point:final, echo=F, fig.width=18, fig.cap="Change points in the time series of marriages in Venice">>=
...
for (i in 1:number.of.points) {
  chart <- chart + geom_vline(xintercept=change.point.years[i], linetype="dashed", alpha=0.5, color=rgb(i/number.of.points,0.1,0.1), linewidth=2*(number.of.points+3-i)/number.of.points)
}
chart
@
\end{landscape}

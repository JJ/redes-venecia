...
\begin{reply}
  We have changed the title to {\em Detecting pivotal moments by using
    changepoint analysis of noble marriages during the time of the Republic of
    Venice}. Since we use single- and multi- change point analysis, we thought
  it was better to not refer to just one of the techniques used. On the other
  hand, we think it is important that we arrive at history from statistics, so
  we have changed {\em turning points} to {\em pivotal moments}, which also
  avoids repetition of the word {\em points} in the title.

  We have also used {\em changepoint} (in the title and throughout all the
  paper) as a denomination for the points of change that have been statistically
  detected; this is one of the generally accepted terms for the mathematical
  concept \cite{killick}.
\end{reply}
...
\begin{reply}
  ...
  is now Table \ref{tab:histories2.summary}. We have also changed from an environment that has an itemized
...
\begin{reply}
  ...
  \ref{fig:histories2.self.marriages}. We observe precisely this peak, within an
  overall low number of self-marriages, but which anyway is statistically
  different from the previous period. We have included text explaining this in
  the text, and thank you again for the suggestion.
\end{reply}
...
\begin{point}
  {\bf Fifth period – 1655-1684} - characterized by an entropy and non-noble
  marriages increase and a decrease in the total number of marriages. Here one
  can detect the results of the first Cretan War aggregation and a new
  phenomenon: inter marriages between Venetian nobles and families not yet
  aggregated, as a strategy on the one hand to enrich the noble branch by the
  dowries of the non-noble family, and on the other hand, as a long-term
  strategy of the non-noble family to establish fruitful relationships inside
  the nobility in order to create a consensus on its candidacy at the time of
  its aggregation (Raines 2006).
\end{point}

\begin{reply}
  These details and references have been added to the manuscript, as well as
  \cite{gelder2013gaining}, which also explains how other wealthy families,
  unrelated to the two wars in the period, were also added; an explanation of
  changes in entropy and marriages to non-noble brides using them is also added.
\end{reply}

\begin{point}
  {\bf Sixth period – 1684-1797} - characterized by a decrease in the overall
  number of marriages and increase of entropy and non-noble marriages. Indeed,
  this period witnesses the system slowly changing. The aggregations (in 1684
  starts a new one) of 125 families changes the rules of the game. As of the
  1720s Venetian nobles do not find politics so attractive for their career or
  for having economic privileges and look therefore for other ways of
  contracting marriages with rich families, even if not from nobility. See
  Hunecke (1997), Sabbadini (1995).
\end{point}

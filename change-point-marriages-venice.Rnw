...
<<change.point:final, echo=F, fig.cap="Change points in the time series of marriages in Venice", fig.align="center", fig.pos="htbp">>=
...
for (i in 1:length(change.point.years)) {
  chart <- chart + geom_vline(xintercept=change.point.years[i], linetype="dashed", color="red", size=length(change.point.years)-i)
}
chart
@

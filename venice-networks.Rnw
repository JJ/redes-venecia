...
<<colleganza,  fig.pos="h!tbp", echo=FALSE, message=F, warning=F, fig.height=5, fig.cap="The \\protect\\emph{Biggest component of the colleganza} network. Family names are sized according to the pagerank centrality, nodes according to the degree, edges according to edge betweenness \\protect\\label{fig:colleganza} ">>=

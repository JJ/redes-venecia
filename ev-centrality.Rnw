...
Many complex networks, including social networks, include links from one actor to itself; these links are often dropped to perform analysis of centrality measures, despite the fact that some of them, like eigenvector centrality, can work with these self-loops. In this paper, we will be looking at the difference of the results in the analysis of a social network using eigenvector centrality when these self loops are considered, and what additional insights we can gather from it, including possible recommendations for the incorporation of these self-loops in more general social network analysis.

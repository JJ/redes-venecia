...
We will try and answer RQ4 by measuring the change point in this specific series \rev{too}.%
%
<<histories2.change.point:entropy2, warning=F, echo=F, fig.height=5, fig.cap="\\protect\\rev{Representation of the entropy time series together with change points found by different methods: red and green found on the annual number of marriages via Lanzante and average/variance difference maximization; dark red is the change point found via Lanzante on this time series.}">>=
...
\rev{The change point at the end of the period is the second most important}. The introduction of the laws mentioned in the previous period caused an increment in the number of marriages, no change in entropy, but mainly resulted in a decrease of the marriages that included a non-noble partner.
...
The period starts with the second weakest change point in the series, \rev{ending, however, with the most important change point, which in fact divides the time series in two}. The number of marriages decreases, entropy increases slightly, and the number of non-patrician marriages increases.
...
\begin{table}[ht]
\caption{\rev{Summary of type of events that have caused the change points}}
...
\end{table}
%
\rev{The changes in every year identified as a change point are shown in Table \ref{tab:histories2.summary}}. \rev{These periods give} give the longest answer to  research question 3, whether shift points match historical events, \rev{in the sense that we can find law enactments, trends or external events that might have caused an epochal shift of the nature evidenced by the statistical analysis}. In two cases, \rev{ends of the second and third period}, shifts were provoked by changes in legislation; the others were caused by external events (mainly conflicts) but supported by legislation (the fact that citizens could become nobles by paying a fee, for instance), \rev{but also, in many cases, convergence of several different trends}. Technological and other changes are in the background, but, by themselves, cannot cause a shift in a time series of social data such as this one.

\rev{What we need to take into account is that what we are looking at is a time series of {\em noble} marriages. Being the ruling class in the Republic of Venice, changes in its composition or social dynamics are bound to affect the rest of the state in the medium or long run; but, first and foremost, change points in the marriages time series will be the direct consequence of legislative, economic and cultural changes related to the nobility. The kind of historical causes that have been identified as causes for the change points are summarized in Table \ref{tab:histories2.events}. This Table shows that three of them, 1435, 1525 and 1684, are mainly {\em internal}, in the sense that they have been provoked by changes in legislation related to noble marriages, which may or may not have been caused by external (economic or social) events; identifying them, however, is besides the point\footnote{Although it would be interesting to assess the legislative precision of the Republic and whether the diagnostic and mid- and long-term effects of legislative measures were the intended ones}. The most important change point, detected in 1654, as well as the third most statistically significant, in 1592, can be attributed mainly to external events that affected the state at large, and nobles (and their marriages) as a part of it, even more so than other classes, since a bigger proportion of the nobility fought (as officials) in the wars that occurred in these periods.}

At any rate, using multi-change point analysis allows a more {\em natural} division of the history of a certain state of region in periods, focusing on the effects of internal (\rev{in the strict sense of belonging to the state, or pertaining to the specific kind of event that the time series represents, noble marriages in this case}) or external events (or combinations thereof), rather than on (possible) causes without demonstrated \rev{quantifiable} effects.

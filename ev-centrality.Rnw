...
This dataset has been chosen because it includes many intra-family marriages; a ranking of the families according to the number of marriages is shown in Table \ref{tab:top:ev} (left). . The dataset is normalized to include only the family bride and groom belong to.
...
# Compute the number of marriages where wife_familyname_std == husband_familyname_std per century
all.marriages %>% group_by(str_sub(all.marriages$year,1,2)) %>% summarise(intra.marriages=sum(wife_familyname_std==husband_familyname_std))
